...
<body <?php body_class(); ?>>
        ...
        <header id="masthead"
                ...
                <div class="max-w-7xl mx-auto flex flex-col items-center gap-4 px-6 py-4 md:flex-row md:justify-center">
                        ...
                        <?php
                        /* ----------  Site logo and title ------------------- */
                        $logo_classes  = 'no-underline text-inherit decoration-transparent transition-transform duration-150 hover:scale-105 hover:opacity-80 active:scale-95 active:text-neutral-900 inline-flex items-center'; // Added active state for click feedback
                        // Add larger text size if on front page
                        if (is_front_page()) {
                                $name_classes = 'no-underline text-inherit decoration-transparent transition-transform duration-150 hover:scale-105 hover:text-neutral-600 hover:opacity-80 active:scale-95 active:text-neutral-900 text-xl md:text-2xl font-semibold'; // Add active state
                        } else {
                        ...
                        <nav class="flex flex-wrap gap-4 justify-center">
                                ...
                                foreach ($links as $l) {
                                        $classes = 'no-underline text-inherit decoration-transparent transition-transform duration-150 hover:scale-105 hover:text-neutral-600 active:scale-95 active:text-neutral-900'; // Active state for nav links
                                        if ($l['check']) {
